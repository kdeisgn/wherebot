...
# Stretch Respeaker Configurations
...
# parameter list -> name: (id, offset, type, max, min , r/w, info)
# ~ Most interested parameter would be VOICEACTIVITY and SPEECHDETECTED ~
PARAMETERS = {
...
''' Class to read/modify the paramaters '''
class Tuning:
    TIMEOUT = 100000
...
##################################################
##### Above code is developed by Stretch Inc. ####
##################################################
...
class StretchAudio:
    def __init__(self):
        try:
            ...
            print("[INFO] Initialized Stretch Audio")
    ...
    def talk_to_stretch(self):
        if self.dev:
            ...
            while True:
                try:
                    if not self.printed_wait_statement and respeaker.is_voice() == 0:
                        print("[INFO] Waiting for audio...")
                        self.printed_wait_statement = True
                    else:
    ...
    def record_audio(self, seconds=3):
        '''
            Records audio using Pyaudio for a specific duration. Default 3 seconds.
    ...
    def record_audio_until_end_of_speech(self, respeaker):
        '''
            Records audio until end of speech. End of speech - silent for 2 seconds.
            Input: Tuning class object (respeaker) to access "is_voice()" function.
            Output: frames.
        '''
        ...
        while True:
            try:
                ...
                # Process audio data for the first channel (fused)
                if sys.version_info[:3] > (3, 0):
                    a = np.frombuffer(data, dtype=np.int16)[0::6] # Extracts fused channel 0
                    frames.append(a.tobytes())
                else:
                ...
                if respeaker.is_voice() == 0:
                    silence_counter += 1 # Increment counter if no speech detected
                else:
                    silence_counter = 0 # Reset counter if speech detected

                # Stop recording if no speech for 2 seconds
                if silence_counter >= 40:
                    print("[INFO] Speech ended")
                    break
                
                time.sleep(0.01) # Add small delay. Higher delay -> larger chunk size -> slow processing
            except Exception as e:
                print(f"[ERROR] Error while recording: {e}")
                break
    ...
    def save_audio(self, frames, fname="user_audio.wav"):
        '''
            Saves the recording into a audio file.
            Input: frames, file name.
            Output: audio file 'user_audio.wav'
        '''
    ...
    def load_audio(self, fname="llm_audio.wav"):
        '''
            Loads an audio file and converts them into frames.
            Input: audio file.
            Output: frames.
        '''
    ...
    def play_audio(self, frames):
        '''
            Plays the audio given the frames.
            Input: frames.
            Output: frames.
        '''
